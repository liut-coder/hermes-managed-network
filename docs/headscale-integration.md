...
审批网关优先使用通用 `approval-gateway` 抽象：installer 会写入 `/etc/hermes-managed-network/approval-gateway.env`，并启用 `hermes-managed-network-approval-gateway.service`。`HMN_TELEGRAM_CHAT_ID`、`HMN_TELEGRAM_BOT_TOKEN`、旧 `telegram-gateway` 命令和 `hermes-managed-network-telegram-gateway.service` 仍保留兼容。
...
- 通用 approval-gateway 一键 systemd 集成，并保留 Telegram Gateway 兼容入口
