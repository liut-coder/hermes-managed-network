...
- [>] 服务部署文档自动填充：后台实现中（worktree `/tmp/hmn-docs-center-automation`），完成后回填。
- [>] 服务维护文档自动生成：后台实现中（worktree `/tmp/hmn-docs-center-automation`），完成后回填。
- [>] 文档集中落地到文档中心：后台实现中（worktree `/tmp/hmn-docs-center-automation`），完成后回填。
