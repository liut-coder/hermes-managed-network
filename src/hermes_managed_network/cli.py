...
def _show_interactive_menu(db: Path | None = None) -> None:
    ...
    while True:
        typer.echo("")
        typer.echo("HMN 控制台")
        typer.echo("1) wake       接入新机器")
        typer.echo("2) node list  查看节点")
        typer.echo("3) audit list 查看审计")
        typer.echo("4) token      创建 token")
        typer.echo("5) update     更新主控")
        typer.echo("6) uninstall  卸载主控")
